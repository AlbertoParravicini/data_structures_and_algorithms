...
\chapter{Algorithms for computing bidimensional convex hulls}
...
It can be seen that the logarithmic model is respected for high number of points, while small discrepancies can be found for sets of point of small size.
This behaviour is probably caused by software overheads
...
By generating the faces of the hull through a \textit{breadth first visit} or a \textit{depth first visit}, and by using a dictionary to check if a face has already been included to the hull, it is possible to generate the entire convex hull in $O(nh)$ time.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.4\textwidth, center, keepaspectratio=1]{{"gift_wrapping_3d"}.pdf}
	\caption{One step of the tridimensional gift wrapping algorithm}
\end{figure}

\subsection{Preparata and Hong's Algorithm for 3D Hulls}
Instead of \textit{Graham's scan}, it is required to compute the partial hulls $H_1,\ \ldots,\ H_{\lceil n/m \rceil}$ with an $O(n\ log\ n)$ algorithm that supports tridimensional points. \\
One possible choice is \textit{Preparata and Hong's algorithm} \cite{Preparata:1977:CHF:359423.359430}.\\
The algorithm uses a \textbf{divide-and-conquer} approach to compute the convex hull: points are recursively split in two groups, until the base case (i.e. a tetrahedron) is found. Then, the groups are merged, two-by-two.

The merging approach works as follow:
given two non-intersecting convex polyhedrons $A$ and $B$, initially find a tangent line $ab$ from $A$ to $B$, by projecting them on a bidimensional plane and applying a bidimensional algorithm for computing tangents.\\
The goal is to wrap $A$ and $B$ is a sort of "cylinder" of triangular faces. To do so, build a triangle $abc$ by picking a point $c \in A$ or $c \in B$. $c$ is picked following a principle similar to \textit{gift-wrapping}; from a face $f$ of the "cylinder", an edge $e_J$ is chosen: the new face to be added will be the one touching $e_j$ that maximizes the angle with $f$. \\
Reiterate to build more triangles, until the edge $ab$ is found again. Then, discard the portions of $A$ and $B$ that are "hidden" by the cylinder.

One merging step takes $O(n)$, and because the sets of points are split in two at each step, the recursion has form
$T(n) = 2T(\frac{n}{2}) + O(n)$, which gives complexity $T(n) = O(n\ log\ n)$, according to the \textit{Master Theorem}.
...
\begin{algorithm}[H]
	...
	\DontPrintSemicolon
	\SetAlgoVlined
	\Input{a list $S$ of tridimensional points, the parameters $m, H$}
	\Output{the convex hull of the set, sorted counterclockwise, or an empty list, if $H$ is $<$ $h$}
	Partition $S$ into subsets $S_1,\ \ldots,\ S_{\lceil n/m \rceil}$.\\
	\For{$i = 1,\ \ldots,\  \lceil n/m \rceil$}
	{
...
\begin{algorithm}[H]
	...
	\DontPrintSemicolon
	\SetAlgoVlined
	\Input{a list $S$ of tridimensional points}
	\Output{the convex hull of the set}
	\For{$i = 1,2,\ \ldots$}{
		L = ChanHullStep3D(S, m, H), where $m = H = min\{|S|, 2^{2^i}\}$\\
		\lIf{L $\ne$ incomplete}{return L}
	}
